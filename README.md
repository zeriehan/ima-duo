...
| 同步设置 | 文件名作为笔记标题 | 见下节 |
| 自动同步 | 启用 / 同步间隔（分钟）/ 同步内容 / 立即同步一次 | 按间隔自动跑「全部推送 / 全部拉取」；可关闭，见下节 |
...
不想每次都手动点「全部推送 / 全部拉取」时，打开**自动同步**：插件按你设的间隔，自动依次跑一遍。

| 设置 | 说明 |
| --- | --- |
| 启用 | 总开关。关闭时**不起任何定时器**，行为与以前完全一致 |
| 同步间隔（分钟） | 最小 1 分钟。改动后**立即**重建定时器，不用重启 Obsidian |
| 同步内容 | `推送 + 拉取`（默认，最省心）/ `仅推送` / `仅拉取` |
| 立即同步一次 | 不等间隔到点，马上按当前设置跑一轮（不影响计时器） |

- **入口有两处**：设置页「自动同步」小节，以及**侧边栏同步面板底部的同名小设置条** —— 不用专门跑一趟设置页。
- **三条守卫**：上一轮没跑完、或你正在手动推 / 拉 → 本轮跳过；没有任何可跑的分身 → 跳过（只有手动触发时才提示"没有可运行的同步分身"）。
- **安静运行**：自动同步期间**不弹**「推送完成 / 拉取完成」通知（否则每 N 分钟刷一次屏），进度看左下角状态栏；只有出错才弹通知。
- **启动补跑**：Obsidian 启动后约 20 秒会补跑一轮，省得开起来还要干等一个间隔。
- 默认**关闭**。
...
│                                               │
│ 自动同步        每 30 分钟 · 推送 + 拉取 · 上次 14:20│
│  启用 [开]   间隔 [30] 分钟   内容 [推送 + 拉取 ▾] │
└───────────────────────────────────────────────┘
```
...
- 再往下是**自动同步**小设置条：开关 / 间隔 / 内容，改完立即生效，不用去设置页。（详见上面「自动同步」一节。）
...
- `运行全部拉取分身`
- `运行指定拉取分身`：挑一个分身跑
- `立即自动同步一次（推送 / 拉取）`：不等间隔，马上按自动同步设置跑一轮
- `检查知识库重复条目（生成清单）`
- `检查同步差异（本地 ↔ 知识库，生成清单）`
- `列出待清理条目（旧版本 / 本地已删除）`
